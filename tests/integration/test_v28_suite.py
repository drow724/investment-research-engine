...
def test_suite_runs_four_isolated_paper_lanes_and_stops_at_deadline(monkeypatch, tmp_path):
    monkeypatch.setenv(
        "INVESTMENT_RUNTIME_V28_PAPER_EXPERIMENT_PREFIX", "paper-v2.8-ablation-20260909"
    )
    ...
    with TestClient(create_app()) as client:
        inventory = client.get("/api/v1/experiments/v28").json()
    ...
    # Starting the app again must not reset balances or restart completed experiments.
    with TestClient(create_app()) as client:
        restarted = client.get("/api/v1/experiments/v28").json()
        assert len(restarted) == 4
        assert all(row["status"] == "COMPLETED" for row in restarted)
